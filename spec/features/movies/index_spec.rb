...
RSpec.describe "movies index page", type: :feature do
  before :each do
    ...
    @movie_1 = @director.movies.create!(title: "Pulp Fiction", oscar: true, nominations: 7)
    @movie_2 = @director.movies.create!(title: "Kill Bill: Volume 1", oscar: false, nominations: 0)
    @movie_3 = @director.movies.create!(title: "Django Unchained", oscar: true, nominations: 5)
  end
  it "displays all titles and their attributes" do
    ...
    expect(page).to have_content("Has Oscar: #{@movie_3.oscar}")
    expect(page).to have_content("Nominations: #{@movie_3.nominations}")
  end
